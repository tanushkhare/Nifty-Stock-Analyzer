...
def print_menu():
    ...
    print("="*60)
    print("1. Single Stock Analysis (price + SMA + volume + RSI 7/14/28 + MACD)")
    print("2. Multiple Stock Comparison (3M % performance + RSI(14) + trend + MACD)")
...
# ---------- SINGLE STOCK WITH RSI(7/14/28) + MACD ----------
...
# ---------- MULTIPLE STOCKS WITH RSI(14) + MACD + TREND ----------
...
def multiple_stock_comparison():
    ...
    # MACD(12,26,9) – show only MACD line for each selected stock
    for s in syms:
        close = close_panel[s].dropna()
        macd_line, signal_line, hist = macd(close, 12, 26, 9)

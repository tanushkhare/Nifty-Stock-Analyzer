...
def print_bullish_scenarios():
    ...
    print("\n=== BULLISH Scenario Ranking (by win-rate) ===")
...
def print_bearish_scenarios():
    ...
    cols = ["Direction", "signal", "action", "win_low", "win_high", "win_avg", "risk"]
    print("\n=== BEARISH Scenario Ranking (by win-rate) ===")
...
# ---------- classify custom list: Have Stock / No Stock ----------
def classify_stock_list_bull_bear():
    print("\nCHECK IF STOCK HAS CLEAR TREND (RSI+MACD)")
    print("Enter stock numbers (space-separated, max 10).")
    ...
    for x in inp:
        if x.isdigit():
            ...
            if 1 <= v <= len(nifty50_symbols):
                idxs.append(v - 1)
    idxs = list(dict.fromkeys(idxs))[:10]
    if not idxs:
        print("No valid choices.")
        return

    syms = [nifty50_symbols[i] for i in idxs]
    print("Selected:", ", ".join(syms))

    rows = []

    for sym in syms:
        data = yf.download(sym, period="6mo", interval="1d", auto_adjust=True, progress=False)
        if data.empty:
            rows.append({"Symbol": sym, "RSI14": np.nan, "MACD": np.nan,
                         "HistΔ": np.nan, "Scenario": "No data", "Stock": "No Stock"})
            continue
        ...
        if rsi_clean.empty or macd_clean.empty or len(hist_clean) < 2:
            rows.append({"Symbol": sym, "RSI14": np.nan, "MACD": np.nan,
                         "HistΔ": np.nan, "Scenario": "Not enough data", "Stock": "No Stock"})
            continue

        latest_rsi = rsi_clean.iloc[-1]
        latest_macd = macd_clean.iloc[-1]
        hist_slope = hist_clean.iloc[-1] - hist_clean.iloc[-2]

        scenario = classify_rsi_macd_scenario(latest_rsi, latest_macd, hist_slope)

        # Have Stock if scenario has clear bullish or bearish wording
        if ("Bullish" in scenario) or ("Bearish" in scenario):
            stock_flag = "Have Stock"
        else:
            stock_flag = "No Stock"
    ...
    print("\n=== STOCK STATUS TABLE (Custom list) ===")
    print(df_res[["Symbol", "RSI14", "MACD", "HistΔ", "Scenario", "Stock"]].to_string(index=False))
    print("=======================================\n")


# ---------- scan all Nifty 50 and list bullish (TABLE OUTPUT) ----------
def list_bullish_stocks():
    print("\nSCANNING NIFTY 50 FOR BULLISH TREND (RSI+MACD)...")
    rows = []
    ...
    if not rows:
        print("No clear bullish trends right now.")
        return

    df_bull = pd.DataFrame(rows)
    print("\n==== BULLISH TREND STOCKS (RSI+MACD) ====")
    print(df_bull[["Symbol", "RSI14", "MACD", "HistΔ", "Scenario"]].to_string(index=False))
    print("=========================================\n")


# ---------- scan all Nifty 50 and list bearish (TABLE OUTPUT) ----------
def list_bearish_stocks():
    print("\nSCANNING NIFTY 50 FOR BEARISH TREND (RSI+MACD)...")
    rows = []
    ...
    if not rows:
        print("No clear bearish trends right now.")
        return

    df_bear = pd.DataFrame(rows)
    print("\n==== BEARISH TREND STOCKS (RSI+MACD) ====")
    print(df_bear[["Symbol", "RSI14", "MACD", "HistΔ", "Scenario"]].to_string(index=False))
    print("==========================================\n")
...
def print_menu():
    ...
    print("3. Bullish TREND Stocks (scan Nifty 50 with RSI+MACD)")
    print("4. Bearish TREND Stocks (scan Nifty 50 with RSI+MACD)")
    print("5. Check Custom List -> Have Stock / No Stock (RSI+MACD)")
    print("6. Bullish / Bearish Scenario Tables (PVOI win-rate)")
    print("0. Exit")
    print("-"*60)
...
def multiple_stock_comparison():
    ...
    raw = yf.download(syms, period="3mo", interval="1d", auto_adjust=True, progress=False)[["Close"]]
    if raw.empty:

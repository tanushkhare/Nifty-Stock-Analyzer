...
def plot_bullish_bearish_comparison():
    ...
    ax.set_title("Bullish vs Bearish Scenario Win-Rate Comparison")
...
# ---------- BULLISH vs BEARISH STOCK GRAPH (3M PERFORMANCE) ----------
def plot_bullish_bearish_stocks():
    print("\nBULLISH / BEARISH STOCK PERFORMANCE (3 Months)")
    ...
    if not bullish_syms and not bearish_syms:
        print("None of the selected stocks classified clearly as bullish or bearish.")
        return
    ...
    if bullish_syms:
        for s in bullish_syms:
            ax_bull.plot(norm.index, norm[s].values, label=s.replace(".NS", ""), linewidth=1.5)
        ax_bull.set_title("Bullish Stocks - 3 Month % Performance")
    ...
    if bearish_syms:
        for s in bearish_syms:
            ax_bear.plot(norm.index, norm[s].values, label=s.replace(".NS", ""), linewidth=1.5)
        ax_bear.set_title("Bearish Stocks - 3 Month % Performance")
...
def print_menu():
    ...
    print("3. Bullish Scenarios Ranking (PVOI + RSI + MACD)")
    print("4. Bearish Scenarios Ranking (PVOI + RSI + MACD)")
    print("5. Bullish vs Bearish Win-Rate Graph (scenarios)")
    print("6. Bullish vs Bearish STOCK Graph (3M performance)")
    print("0. Exit")
    print("-"*60)

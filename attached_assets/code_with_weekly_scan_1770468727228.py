...
# ---------- NEW: Full Nifty 50 Scanner ----------
def scan_all_nifty50():
    print("\n SCANNING ALL NIFTY 50 STOCKS (Weekly)...")
    ...
    raw_data = yf.download(nifty50_symbols, period="1y", interval="1wk", auto_adjust=True, group_by='ticker')
    ...
    for symbol in nifty50_symbols:
        try:
            ...
            if len(close) < 30:  # Need minimum data
                neutral_stocks.append((symbol, "Not enough data"))
                continue
            ...
            elif "Bearish" in scenario or "Bearish" in scenario:
    ...
    # Display Results
    print("\n" + "="*80)
    print(" NIFTY 50 WEEKLY SCAN RESULTS")
    print("="*80)
...
def print_menu():
    print("\n" + "="*70)
    print("           NIFTY 50 WEEKLY ANALYZER v2.0")
    print("="*70)
    print("1.  Scan ALL Nifty 50 (Bullish/Bearish/Neutral)")
    print("2.  Single Stock Weekly Analysis (PVOI + RSI + MACD)")
    print("3.  Multiple Stock Weekly Comparison (6M)")
    print("4.  Bullish Scenarios Ranking")
    print("5.  Bearish Scenarios Ranking")
    print("6.  Bullish vs Bearish Win-Rate Graph")
    print("7.  Bullish vs Bearish STOCK Graph (6M)")
    print("0.  Exit")
    print("-"*70)
...
def single_stock_analysis():
    ...
    data = yf.download(symbol, period="2y", interval="1wk", auto_adjust=True)
    if data.empty:
...
def multiple_stock_comparison():
    ...
    raw = yf.download(syms, period="6mo", interval="1wk", auto_adjust=True)[["Close"]]
    if raw.empty:

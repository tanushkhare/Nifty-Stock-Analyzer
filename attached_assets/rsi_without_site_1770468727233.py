...
def print_menu():
    ...
    print("2. Multiple Stock Comparison (3M % performance + RSI(14) + trend)")
...
# ---------- MULTIPLE STOCKS WITH RSI(14) + TREND ----------
...
def multiple_stock_comparison():
    ...
    for s in syms:
        ...
        rsi14 = rsi(close, 14)
        ax_rsi_ax.plot(rsi14.index, rsi14.values, label=f"{s.replace('.NS','')} RSI14", linewidth=1.0)
    ...
    # Trend summary based on SMA20 and SMA50
    print("\nTrend for selected stocks (using price vs SMA20 vs SMA50):")
    for sym in syms:
        try:
            close = close_panel[sym].dropna()
            if len(close) < 60:
                print(f"{sym}: Not enough data for trend.")
                continue
            ...
            if latest_price > latest_sma20 and latest_sma20 > latest_sma50:
                ...
                trend = "Short‑term bounce above SMA20; longer trend still weak."
            elif latest_price < latest_sma20 and latest_sma20 > latest_sma50:
                trend = "Pullback within an uptrend (price below SMA20, above SMA50)."
            elif latest_price >= latest_sma50 and latest_sma20 <= latest_sma50:
                trend = "Sideways / base formation around SMA50."
            else:
                trend = "Downtrend or weak trend (price below SMAs)."

            print(f"{sym}: {trend}")
        except Exception as e:
            print(f"{sym}: Error computing trend ({e}).")
            continue
